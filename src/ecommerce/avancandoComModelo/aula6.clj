...
(produto/upsert! conn [camera celular calculadora notebook] "127.0.0.1")

(produto/atribui-categoria! conn [camera celular notebook] eletronicos)

;insert with nested map
(produto/upsert! conn [{:produto/id                (model/uuid)
...
(produto/mais-caro (d/db conn))
(produto/mais-barato (d/db conn))
